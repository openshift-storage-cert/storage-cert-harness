...
require_cmd go
# ci-utils sets GOPROXY=off for vendor builds; allow toolchain auto-download when
# the local `go` binary is older than go.mod (developer laptops without go-toolset).
if [[ "${GOPROXY}" == "off" ]]; then
