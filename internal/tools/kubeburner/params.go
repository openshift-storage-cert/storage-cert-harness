...
func resolveParams(trs []core.TestRequirement) Params {
	p := Params{VMImage: DefaultVMImage, DiskGiB: defaultDiskGiB, QPS: defaultQPS, Burst: defaultBurst, MaxWait: defaultMaxWait, NS: defaultNS}
	...
	if n, ok := core.AsInt(raw["disk_gib"]); ok && n > 0 {
		p.DiskGiB = n
	}
