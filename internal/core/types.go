...
type Verdict struct {
	...
	Scenario  string  `json:"scenario,omitempty"` // run label (e.g. "under-pressure"); distinguishes merged runs of the same TR
...
func ExecutionLabel(id, variant string) string {
	if variant == "" {
		return id
	}
	return id + " [" + variant + "]"
