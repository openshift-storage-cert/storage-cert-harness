...
func TestVariantValidation(t *testing.T) {
	for _, body := range []string{
		...
		if _, err := Load(path); err == nil {
			t.Fatalf("accepted invalid variants: %s", body)
		}
	}
}

func TestLoadNamedVariants(t *testing.T) {
	path := filepath.Join(t.TempDir(), "plan.yaml")
	data := "schema_version: \"1\"\nname: variants\nselect:\n  ids: [TR-1]\nvariants:\n  TR-1:\n    - name: small\n      params:\n        mode: light\n    - name: large\n"
	if err := os.WriteFile(path, []byte(data), 0o600); err != nil {
		t.Fatal(err)
	...
	vs := p.Variants["TR-1"]
	if len(vs) != 2 || vs[0].Name != "small" || vs[0].Params["mode"] != "light" || vs[1].Name != "large" {
		t.Fatalf("incorrect variants: %+v", vs)
	}
}
